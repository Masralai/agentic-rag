...
import { addSource, removeSource, listSources, listMessages, generateSummary, renameNotebook } from "../../actions";
import { FileText, Send, Plus, X, Loader2, BookOpen, HelpCircle, File as FileIcon } from "lucide-react";
    ...
    <div className="flex flex-1 min-h-0">
      ...
      <aside className="w-80 shrink-0 border-l border-zinc-900 flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-1">
          <p className="text-xs text-zinc-600 px-2 pb-2 font-medium">SOURCES</p>
          {sources.length === 0 && (
            <p className="text-xs text-zinc-700 px-2">No sources yet</p>
          )}
          {sources.map((src) => {
            const Icon = fileTypeIcons[src.type] || FileText;
            return (
              <div key={src.id} className="group flex items-center gap-3 px-3 py-2.5 hover:bg-zinc-900 rounded-lg transition-colors">
                ...
                <button onClick={() => handleRemoveSource(src.id)} className="opacity-0 group-hover:opacity-100 text-zinc-600 hover:text-red-400 transition-all">
                  <X size={14} />
                </button>
              </div>
            );
